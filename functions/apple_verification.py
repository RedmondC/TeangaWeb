...
def handle_subscription_event(decoded_data):
    ...
    if is_purchase:
        update_subscription_status([convert_apple_notification_to_subscription(decoded_data, action_title, description)])
...
def parse_notification(data: dict) -> (bool, str, str):
    ...
    if notification_type in ["SUBSCRIBED"]:
        return True, "Product Purchased", f"Subscription started for product {subscription_id} with order ID {transaction_id}."
    elif notification_type in ["DID_RENEW"]:
        return True, "Product Renewed", f"Subscription renewed for product {subscription_id} with order ID {transaction_id}."
    elif notification_type in ["RENEWAL_EXTENDED", "REFUND_REVERSED"]:
        return True, "Special action", f"The special action {notification_type} has been performed for product {subscription_id} with order ID {transaction_id}."

    return False, None, None

def convert_apple_notification_to_subscription(decoded_data: dict, action_title: str, history_description: str) -> Subscription:
